...
public class GesturesActivity extends AppCompatActivity {
    ...
    private void loadGestureImages() {
        try {
            // Load images for numbers 1-10
            for (int i = 1; i <= 10; i++) {
                String imagePath = String.format("hand_signs_images/asl_numbers/%03d.png", i);
                ImageView imageView = findViewById(getResources().getIdentifier(
                    "gesture_image_" + i, "id", getPackageName()));
                ...
                // Load custom label if exists, otherwise use original
                TextView labelView = findViewById(getResources().getIdentifier(
                    "gesture_label_" + i, "id", getPackageName()));
                String customKey = KEY_CUSTOM_PREFIX + i;
                String originalMeaning = gesturePrefs.getString(KEY_ORIGINAL_PREFIX + i, String.valueOf(i));
                String customMeaning = gesturePrefs.getString(customKey, originalMeaning);
                labelView.setText(customMeaning);
            }

            // Load Y gesture image
            ImageView yImageView = findViewById(R.id.gesture_image_y);
            InputStream is = getAssets().open("hand_signs_images/asl_alphabet/Y.png");
            Bitmap bitmap = BitmapFactory.decodeStream(is);
            yImageView.setImageBitmap(bitmap);
            is.close();

            // Load custom Y label if exists, otherwise use original
            TextView yLabelView = findViewById(R.id.gesture_label_y);
            String customKey = KEY_CUSTOM_PREFIX + "y";
            String originalMeaning = gesturePrefs.getString(KEY_ORIGINAL_PREFIX + "y", "Y");
            String customMeaning = gesturePrefs.getString(customKey, originalMeaning);
            yLabelView.setText(customMeaning);
